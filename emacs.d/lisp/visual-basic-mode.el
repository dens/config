...
;; 
;; DST 2008-09-16: indent, tabs
;; DST 2009-01-23: single-line ifs, abbrevs SYS-Flags, "_" am Funktionsanfang
;;
...
(eval-and-compile
  ...
  '("Add" "Aggregate" "And" "App" "AppActivate" "Application" "Array" "As"
    ...
    "Enum" "Environ" "Erase" "Err" "Error" "Exit" "Exp" "Explicit"
    "FV" "False" "Field"
...
\\{visual-basic-mode-map}"
  ...
  ;; (add-hook 'local-write-file-hooks 'visual-basic-tabify)
...
;;         (setq word (car words)
;;               words (cdr words))
;;         (setq list (cons (list (downcase word) word nil 0 t) list)))
;; 
...
(defun visual-basic-construct-keyword-abbrev-table ()
  (let ((words visual-basic-all-keywords)
    ...
    (while words
      (setq word (car words)
            words (cdr words))
      (define-abbrev visual-basic-mode-abbrev-table
          (downcase word) word nil :system t))))
            ...
            (t
             ...
             (cond
              ((looking-at visual-basic-continuation-regexp)
               ...
               ;; or else one word in.
               (if (or (looking-at visual-basic-defun-start-regexp)
                       (looking-at visual-basic-if-regexp)
                       (looking-at visual-basic-while-regexp)
                       (looking-at visual-basic-for-regexp)
                       (looking-at visual-basic-select-regexp))
                   (+ (current-indentation) (* 2 visual-basic-mode-indent))
                   (+ (current-indentation) visual-basic-mode-indent))
